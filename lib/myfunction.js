...
export const processTime = (timestamp, now) => moment.duration(now - moment(timestamp * 1000)).asSeconds()
...
export const getGroupAdmins = (participants) => participants.filter(i => i.admin === "superadmin" || i.admin === "admin").map(i => i.id)
  ...
  if (m.message) {
    ...
    if (m.quoted) {
      ...
      if (m.quoted) {
        ...
        m.quoted.delete = () => conn.sendMessage(vM.key.remoteJid, { delete: vM.key })
