...
import "../../lib/settings/config.js"; // Untuk mengakses global.appearance

// Set timezone to WIB (Jakarta)
moment.tz.setDefault(global.appearance.timezone || "Asia/Jakarta");

export const getWIBTime = (format = global.appearance.timeFormat || "HH:mm:ss") => {
  return moment().format(format);
};

export const getWIBDate = (format = global.appearance.dateFormat || "DD/MM/YYYY") => {
  return moment().format(format);
};

export const getWIBDateTime = (format = global.appearance.fullDateFormat || "DD/MM/YYYY HH:mm:ss") => {
  return moment().format(format);
};
...
// Fungsi waktu lengkap (jika dibutuhkan di masa depan)
